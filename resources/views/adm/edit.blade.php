...
@section('content')
    <section class="flex flex-col gap-5">
        <h1 class='lg:text-center capitalize'>Edit {{ $car->name }} as
            <span class="text-primary">{{ auth()->user()->company_name }}</span>
        </h1>
        <form class="flex flex-col gap-4" method="POST" action="{{ route('admin.edit', $car->id) }}">
            @csrf
            @method('POST')
            ...
            <div class="flex lg:flex-row flex-col gap-5">
                <label class="flex flex-col gap-2 w-full">
                    ...
                    <input type="text" autocomplete='off' name="name" id="name" value="{{ $car->name }}"
                        required>
                </label>
                <label class="flex flex-col gap-2 w-full">
                    <span class="text-xl">Price</span>
                    <input type="number" max="10000000" min="1000" autocomplete='off' name="price" id="price"
                        value="{{ $car->price }}" required step="0.001">
                </label>
            </div>
            <div class="flex lg:flex-row flex-col gap-5">
                <label class="flex flex-col gap-2 w-full">
                    <span class="text-xl">Description</span>
                    <input type="text" maxlength="512" autocomplete='off' name="description" id="description"
                        value="{{ $car->description }}" required>
                </label>
                <label class="flex flex-col gap-2 w-full">
                    <span class="text-xl">Brand</span>
                    <input type="text" maxlength="80" autocomplete='off' name="brand" id="brand"
            ...
            <div class="flex lg:flex-row flex-col gap-5">
                <label class="flex flex-col gap-2 w-full">
                    ...
                    <select autocomplete='off' name="color" id="color" value="{{ $car->color }}" required
                        class=" text-white">
                ...
                <label class="flex flex-col gap-2 w-full">
                    ...
                    <select autocomplete='off' value="{{ $car->type }}" name="type" id="type" required
                        class=" text-white">
            ...
            <div class="flex lg:flex-row flex-col gap-5">
                <label class="flex flex-col gap-2 w-full">
                    ...
                    <select autocomplete='off' value="{{ $car->state }}" name="state" id="state" required
                        class=" text-white">
                ...
                <label class="flex flex-col gap-2 w-full">
                    ...
                    <select autocomplete='off' value="{{ $car->transmission }}" name="transmission" id="transmission"
                        required class=" text-white">
            ...
            <div class="flex lg:flex-row flex-col gap-5">
                <label class="flex flex-col gap-2 w-full">
                    ...
                    <input type="number" value="{{ $car->km }}" autocomplete="off" name="km" id="km"
                        required>
                </label>
                <label class="flex flex-col gap-2 w-full">
                    <span class="text-xl">Year</span>
                    <input type="text" value="{{ $car->year }}" autocomplete="off" name="year" id="x"
                        required>
                </label>
            </div>
